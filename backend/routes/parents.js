...
router.get('',verifyToken,async (req,res)=>{
    ...
    let results =[]
    let idsSA = await Role.find({type : 'Parent'},'_id');
...
router.get('/:id',verifyToken,async (req,res)=>{
    let user = await User.findById(req.params.id).populate('role').populate('directeur.etablissement');
    if(!user)
...
router.post('',verifyToken,async (req,res)=>{
    ...
    let user = await new User(_.pick(req.body, ['firstName', 'lastName', 'email', 'password', 'confirmpassword', 'phone','matiere','directeur','etablissement']))
    ...
    try {
        ...
        let r=idsSA[0]
        user.role=r
        user.etablissement=directeur.etablissement
        ...
        var transporter = nodemailer.createTransport({
          ...
          var mailOptions = {
            ...
            text: 'Bonjour '+ user.firstName + ', vous etes inscrit dans notre plateforme E-learning comme étant un Parent : '
