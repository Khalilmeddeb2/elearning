...
router.get('/parent/:idPar',verifyToken,async (req,res)=>{
    let profils = await ProfilEnfant.find({parent :req.params.idPar}).populate('enfant')
...
router.post('/:idEnfant',upload.single('image'),verifyToken,async(req,res)=>{
    ...
    let profilPropreites= await lodashsh.pick(req.body,['enseignant','enfant','description','date'])
