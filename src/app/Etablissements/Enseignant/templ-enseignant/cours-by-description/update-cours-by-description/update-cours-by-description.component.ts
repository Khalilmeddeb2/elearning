...
@Component({
  selector: 'app-update-cours-by-description',
  templateUrl: './update-cours-by-description.component.html',
  styleUrls: ['./update-cours-by-description.component.scss']
})
export class UpdateCoursByDescriptionComponent implements OnInit {
...
getMatieres(){
  ...
  UpdateCours() {
    ...
    //this.loading = true;
    console.log('enabled');
    this.coursService.EditCoursWithDescription(this.id ,this.registerForm.value).subscribe(
      (data) => {
  ...
  goToList(){
    this.router.navigate(['/EnseignantHome/cours' , {caller2 : "Modification avec succès"}] )
    }
    onSubmit(){
      console.log("zzz");
      this.UpdateCours();
